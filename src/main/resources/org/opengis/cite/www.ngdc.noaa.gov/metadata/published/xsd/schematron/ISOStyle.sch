...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron"
  ...
  <!--
    ...
    Updated: 2012-11-14 - added test for fileIdentifier, test for xlinks to Docucomp
  -->
  <sch:pattern>
    <sch:title>Check for fileIdentifier</sch:title>
    <sch:rule context="gmi:MI_Metadata">
      <sch:assert test="gmd:fileIdentifier">Metadata record should have a
        fileIdentifier.</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
    ...
    <sch:rule
      context="//*[ends-with(name(./*[1]),'Code') and count(*)=1 and not(name(.)='gmd:topicCategory')]">
      <sch:assert test="normalize-space(./*[1]) = ./*[1]/@codeListValue">ME: The
        codeListValue attribute must match the content of the codeList
        element</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
    ...
    <sch:rule context="//@uuid">
      <sch:assert
        test="matches(.,'^[0-9|a-f|A-F]{8}[-]?[0-9|a-f|A-F]{4}[-]?[0-9|a-f|A-F]{4}[-]?[0-9|a-f|A-F]{4}[-]?[0-9|a-f|A-F]{12}$')"
        >uuid attributes must be valid uuids</sch:assert>
      <!-- 
   Second test
        test="matches(.,'^[\d|\w]{8}[-]?[\d|\w]{4}[-]?[\d|\w]{4}[-]?[\d|\w]{4}[-]?[\d|\w]{12}$')" 
...
-->
  ...
  <sch:pattern>
    ...
    <sch:rule context="//gmd:CI_OnlineResource">
      <sch:assert test="gmd:name">onlineResources should have a
        name</sch:assert>
      <sch:assert test="gmd:description">onlineResources should have a
        description</sch:assert>
      <sch:assert test="gmd:function">onlineResources should have a
        function</sch:assert>
    </sch:rule>
  </sch:pattern>
  ...
  <sch:pattern>
    ...
    <sch:rule context="//gmx:Anchor">
      ...
      <sch:assert test="$xlinkVoc">
        <sch:value-of select="./attribute::xlink:href"/> is not a valid URL.
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
    <sch:title>Test Xlinks to Docucomp</sch:title>
    <sch:rule context="//gmd:*/@xlink:href">
      <sch:let name="rootURL" value="substring-before(., 'docucomp')"/>
      <sch:assert
        test="matches($rootURL, 'http://www.ngdc.noaa.gov/') or matches($rootURL, 'https://www.ngdc.noaa.gov/')"
        >URL to component in Docucomp is not valid. It should look like:
        http://www.ngdc.noaa.gov/docucomp </sch:assert>
    </sch:rule>
  </sch:pattern>
