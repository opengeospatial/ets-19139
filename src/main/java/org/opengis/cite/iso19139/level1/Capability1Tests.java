...
public class Capability1Tests {
  ...
  public synchronized void validateXMLAgainstXSD(ITestContext testContext) throws SAXException,
    ...
    String failReport = inputFile + " doesn't conform to the clause A.1 of ISO 19139.\n\n";
...
//    params.put(TestRunArg.XSD.toString(), this.getClass().getResource(ETS_ROOT_PKG + "xsd/iso/19139/20070417/gmd/gmd.xsd").toString());
    ...
    if (((periodIndex >= 1) || (periodIndex == -1)) && slashIndex >= 0 && slashIndex < url.length() - 1) {
      ...
      try {
        ...
        testContext.setAttribute("Input", inputFile);
        ...
        try {
          ...
          testResult = true;
          failReport = inputFile + " conform to the clause A.1 of ISO 19139\n";
          result = "PASS";
          testContext.setAttribute("Result", result);
          testContext.setAttribute("TestResultReport", failReport);
...
//                    Print the reason why the XML validation fails
          ...
          failReport = inputFile + " doesn't conform to the clause A.1 of ISO 19139,\n"
                  + "Reason: " + reason
                  + ",\nLine Number \t: " + e.getLineNumber()
                  + ",\nColumn Number\t: " + e.getColumnNumber() + "\n";
          String failReport1 = inputFile + " doesn't conform to the clause A.1 of ISO 19139\n"
                  + "Reason: " + reason
                  + "\nLine Number \t: " + e.getLineNumber()
                  + "\nColumn Number\t: " + e.getColumnNumber() + "\n";
          testContext.setAttribute("TestResultReport", failReport1);

          testResult = false;
        }
      } catch (IOException e) {
        //XML file is corrupted or malformed
        testContext.setAttribute("TestResultReport", "Error in reading or writing from the XML file " + inputFile);
        failReport = "Error in reading or writing from the XML file" + inputFile;
      } catch (SAXException e) {
        //Schema file (XSD) is corrupted or malformed
        testContext.setAttribute("TestResultReport", "Error in reading the XML Schema.");
